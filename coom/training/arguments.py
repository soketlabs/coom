...
def validate_args(args, defaults={}):
    ...
    if args.perform_rl_step:
        ...
        assert num_generated_samples_per_inference_iteration % args.global_batch_size == 0, \
            "grpo_group_size * grpo_prompts_per_step * grpo_iterations should be divisible by global_batch_size"
        ...
        if args.exit_interval is not None:
            assert args.exit_interval % num_training_iterations_per_inference_iteration == 0, \
                "exit_interval should be divisible by number of global batches per inference iteration."
        if args.save_interval is not None:
            assert args.save_interval % num_training_iterations_per_inference_iteration == 0, \
                "save_interval should be divisible by number of global batches per inference iteration."
